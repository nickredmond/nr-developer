...
function navigateQuestions(indexChange) {
	...
	else if (verifyCurrentQuestion(true)){
...
function verifyCurrentQuestion(isErrorsDisplayed) {
	var isComplete = true;

	if (isStringNullOrEmpty(getQuestionText())) {
		isComplete = false;
		if (isErrorsDisplayed) {
...
function isQuizPublishable() {
	return questions.length > 0 &&
		(isCurrentQuestionBlank() || verifyCurrentQuestion(true));
}
function onConfirmClick() {
	if (currentModalContext === "delete") {
		...
		$.post("/create-quiz", JSON.stringify(body))
			.done(function(data) {
				var link = document.getElementById("published-quiz-anchor");
				link.href = "/" + quizName;
			...
			.fail(function(error) {
				if (error["status"] === 400 && error["responseJSON"] && 
						error["responseJSON"]["is_name_taken"]) {
					document.getElementById("taken-quiz-name-label").innerHTML = quizName;
					document.getElementById("taken-name-alert").style.display = "block";
				}
			});
	}
	else {
...
function isCurrentQuestionBlank() {
...
function onPublishQuizClick() {
	refreshView();
	if (verifyQuizName()) {
		if (isQuizPublishable()) {
			if (isCurrentQuestionBlank() || selectedQuestionIndex < questions.length) {
